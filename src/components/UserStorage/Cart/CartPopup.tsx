...
const CartPopup: React.FC<CartPopupType> = ({ setIsMore, isMore }) => {
  ...
  const [isDragging, setIsDragging] = React.useState<boolean>(false); // Отслеживаем движение

  const handleTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    setStartTouch(e.touches[0].clientY);
    setIsDragging(false); // Сбрасываем флаг движения
  };
  ...
  const handleTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    // Игнорируем небольшие движения (например, до 5px)
    if (Math.abs(deltaY) < 5) return;

    setIsDragging(true); // Устанавливаем флаг движения
    if (deltaY > 0) {
  ...
  const handleTouchEnd = () => {
    if (!isDragging) {
      // Если пользователь просто удерживал палец, ничего не делаем
      setStartTouch(null);
      return;
    }

    // Закрываем окно при достаточном свайпе вниз
    if (translateY > 150) {
      setIsMore(false);
    }
    ...
    >
      <div
        ...
        style={{
          ...
          transition: isDragging ? 'none' : 'transform 0.3s ease', // Анимация только при отпускании
        }}
      >
